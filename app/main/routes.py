...
def send_message(recipient):
    ...
    return render_template('main/send_message.html', recipient=recipient, title=_('Send Message'), form=form)
...
def messages():
    ...
    db.session.commit()
    page = request.args.get('page', 1, type=int)
    messages = current_user.messages_received.order_by(Message.timestamp.desc()).paginate(page,
                                                                                 current_app.config['RESULTS_PER_PAGE'],
                                                                                 False)
    next_url = url_for('main.messages', page=messages.next_num) if messages.has_next else None
    prev_url = url_for('main.messages', page=messages.prev_num) if messages.has_prev else None
    return render_template('main/messages.html', prev_url=prev_url, next_url=next_url, messages=messages.items)
